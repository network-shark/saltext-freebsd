...
def _write_options(name, configuration):
    ...
    if not os.path.isdir(dirname):
        try:
            ...
            raise CommandExecutionError(f"Unable to make {dirname}: {str(exc)}") from exc
...
def config(name, reset=False, **kwargs):
    ...
    """
    _portpath = _check_portname(name)
    ...
    opts = {str(key): _normalize(value) for key, value in kwargs.items() if not key.startswith("_")}
