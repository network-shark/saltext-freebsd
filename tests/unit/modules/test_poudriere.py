...
def test_is_jail():
    ...
    with (
        patch.dict(poudriere.__salt__, {"cmd.run": mock}),
...
def test_parse_config():
    ...
    with (
        patch.dict(poudriere.__salt__, {"config.option": mock}),
        patch("salt.utils.files.fopen", mock_open()),
        patch.object(poudriere, "_check_config_exists", MagicMock(side_effect=[True, False])),
    ):
...
def test_create_jail():
    ...
    mock_true = MagicMock(return_value=True)
    with (
        patch.dict(poudriere.__salt__, {"cmd.run": mock_stack}),
        patch("salt.modules.poudriere._check_config_exists", MagicMock(return_value=True)),
    ):
    ...
    with (
        patch.object(poudriere, "make_pkgng_aware", mock_true),
        patch("salt.modules.poudriere._check_config_exists", MagicMock(return_value=True)),
    ):
...
def test_bulk_build():
    ...
    with (
        ...
        with patch.object(os.path, "isfile", MagicMock(return_value=True)):
            assert (
                poudriere.bulk_build("90amd64", "/root/pkg_list") == "Could not find jail 90amd64"
            )

    ret = "There may have been an issue building packages dumping output: 90amd64 stack"
    with (
        patch.object(os.path, "isfile", MagicMock(return_value=True)),
        patch("salt.modules.poudriere._check_config_exists", MagicMock(return_value=True)),
    ):
        mock = MagicMock(return_value="90amd64 stack packages built")
        with patch.dict(poudriere.__salt__, {"cmd.run": mock}):
            assert (
                poudriere.bulk_build("90amd64", "/root/pkg_list") == "90amd64 stack packages built"
            )
